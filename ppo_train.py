        ...
        if stage == 'bc':
            ...
            if (i_episode + 1) % print_every == 0 or i_episode + 1 == num_episode:
                if print_every < 10:
                    ...
                    prt = 10
                logger.info(
                    "Stage: {}, Episode: {}, Reward: {}, agent_num: {}, survival: {}, end_rate: {}, final_pos: {}, pol_loss: {}, val_loss: {}, dis_ag_rew: {}, dis_ex_rew: {}, dis_total: {}".format(
                        stage,
                        i_episode + 1, np.mean(rewards_log[-prt:]), agent_num, np.mean(avg_survival_log[-prt:]),
                        np.mean(ends_rate[-prt:]), np.mean(avg_finals[-prt:]),
                        np.mean(pol_losses[-prt:]),
                        np.mean(val_losses[-prt:]),
                        np.mean(dis_ag_rew[-prt:]),
