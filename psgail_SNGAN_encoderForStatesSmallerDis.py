...
class Net(torch.nn.Module):
    def __init__(self, hidden_size, input_size, output_size, layer_num=2):
        ...
        for i in range(layer_num - 1):
            layers.append(torch.nn.utils.spectral_norm(torch.nn.Linear(last_size, hidden_size)))
...
class NetEncode(torch.nn.Module):
    def __init__(self, hidden_size, input_size, output_size, layer_num=1):
...
class PSGAIL():
    def __init__(self,
        ...
        self.lambda_gp = 10
        self.discriminator = Net(64, 10, output_size=1)
        self.state_encoder = Net(hidden_size, state_space, output_size=8)
    ...
    def update_discriminator(self, batch, sap_experts):
        ...
        states_agent_encoded = self.state_encoder(s)
        a = a.detach()
        sap_agents = torch.cat((states_agent_encoded, a), dim=1)
        states_exp_encoded = self.state_encoder(sap_experts[:, :-2])
        exp_cat = torch.cat((states_exp_encoded, sap_experts[:, -2:]), dim=1)
        D_expert = self.discriminator(exp_cat)
        D_agents = self.discriminator(sap_agents)
        # grad_penalty = self.grad_penalty(sap_agents.data, sap_experts.data)
        discriminator_loss = D_agents.mean() - D_expert.mean()
        discriminator_loss.backward()
        self.discriminator_optimizer.step()
        return float(D_agents.mean().data), float(D_expert.mean().data), float(discriminator_loss)
    ...
    def update_generator(self, batch, gamma):
        ...
        states_encoded = self.state_encoder(s)
        sap_agents = torch.cat((states_encoded, a), dim=1)
        D_agents = self.discriminator(sap_agents)
